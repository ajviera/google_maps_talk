...
Exploring the new Google NavigationSDK for Flutter and future changes to Google Maps
...
_\*Google Maps Talk works on iOS, Android, Web, and Windows._
...
Use this example to create a file to store the environment variables for native (For now we are only using the google maps API key, so it is the same for all environments):
```sh
# iOS
$ cp ios/Flutter/Environment_example.xcconfig ios/Flutter/Environment.xcconfig

# Android
$ cp android/environment_example.properties android/environment.properties
```
