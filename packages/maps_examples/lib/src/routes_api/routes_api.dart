...
String _mapsApiKey = Platform.isAndroid
...
///
/// [waypoints] is a list of [NavigationWaypoint] representing
/// the route waypoints.
Future<List<String>> getRouteToken(List<NavigationWaypoint> waypoints) async {
  assert(
    _mapsApiKey.isNotEmpty,
    'MAPS_API_KEY is not provided. Please pass it as a '
    'Dart define during the app build.',
  );
  assert(
    waypoints.length >= 2,
  ...
  final apiUrl = Uri.parse(_computeRoutesUrl);
  ...
  if (response.statusCode == 200) {
    ...
    final routeTokens = responseData['routes'] as List<dynamic>?;
