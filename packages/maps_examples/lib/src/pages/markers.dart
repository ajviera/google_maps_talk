...
class _MarkersPageState extends ExamplePageState<MarkersPage> {
  ...
  final List<double> _alphas = <double>[1, 0.3];
  ...
  Future<void> addMarkerToMap() async {
    ...
    final position = await _navigationViewController.getCameraPosition();
    ...
    if (addedMarkers.isNotEmpty) {
      final marker = addedMarkers.first!;
      setState(() {
        _markers.add(marker);
  ...
  Future<void> _updateSelectedMarkerWithOptions(MarkerOptions options) async {
    final newMarker = _selectedMarker!.copyWith(options: options);

    final markers =
        await _navigationViewController.updateMarkers(<Marker>[newMarker]);
    final marker = markers.firstOrNull;
    if (marker != null) {
      setState(() {
        _markers = _markers
  ...
  Future<void> _toggleVisibility() async {
    final oldVisibility = _selectedMarker!.options.visible;
  ...
  Future<void> _toggleDraggable() async {
    final oldDraggable = _selectedMarker!.options.draggable;
    await _updateSelectedMarkerWithOptions(
  ...
  Future<void> _toggleFlat() async {
    final oldFlat = _selectedMarker!.options.flat;
    await _updateSelectedMarkerWithOptions(
  ...
  Future<void> _setAlpha() async {
    final oldAlpha = _selectedMarker!.options.alpha;
    final newAlpha = _alphas.elementAtOrNull(
  ...
  Future<void> _setZIndex() async {
    final oldZIndex = _selectedMarker!.options.zIndex;
    final newZIndex = _zIndexes.elementAtOrNull(
  ...
  Future<ImageDescriptor> _getOrCreateCustomImageFromAsset() async {
    ...
    const assetImage =
        AssetImage('packages/maps_examples/assets/images/marker1.png');
    final configuration = createLocalImageConfiguration(context);
    final assetBundleImageKey = await assetImage.obtainKey(configuration);
    final imagePixelRatio = assetBundleImageKey.scale;
    final imageBytes = await rootBundle.load(assetBundleImageKey.name);
  ...
  Future<void> _toggleCustomIcon() async {
    ...
    if (_selectedMarker!.options.icon.registeredImageId == null) {
      final customMarkerIcon = await _getOrCreateCustomImageFromAsset();
      await _updateSelectedMarkerWithOptions(
  ...
  Widget get bottomControls {
    final style = ElevatedButton.styleFrom(minimumSize: const Size(150, 36));

    final imageIdType = _selectedMarker?.options.icon.registeredImageId != null
        ? 'Custom'
        : 'Default';
        ...
        children: <Widget>[
          Wrap(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                style: style,
                onPressed: _selectedMarker == null ? null : _removeMarker,
                child: const Text('Remove marker'),
              ),
              ElevatedButton(
                style: style,
                onPressed: _selectedMarker == null ? null : _toggleVisibility,
