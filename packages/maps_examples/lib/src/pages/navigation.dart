...
import 'package:maps_examples/src/routes_api/routes_api.dart';
...
class _NavigationPageState extends ExamplePageState<NavigationPage> {
  /// If navigation ui is disabled right after navigation session
  /// is initialized,
  ...
  Future<void> _setRouteTokensEnabled(bool value) async {
    setState(() {
      // Route tokens are only supported for the driving mode in this
      // example app.
      _travelMode = NavigationTravelMode.driving;
      _validRoute = false;
      _routeTokensEnabled = value;
    });
    final success = await _updateNavigationDestinations();
    if (success) {
      setState(() {
        _validRoute = true;
      });
  ...
  Future<void> _restorePossibleNavigatorState() async {
    if (_navigatorInitialized) {
      final waypoints = await _getWaypoints();

      // Restore local waypoint index
      if (waypoints.isNotEmpty) {
        final parts = waypoints.last.title.split(' ');
        if (parts.length == 2) {
  ...
  Future<void> _updateNewWaypointMarker(LatLng target) async {
    final markerOptions = _buildNewWaypointMarkerOptions(target);
    if (_newWaypointMarker == null) {
      // Add new marker.
      final addedMarkers = await _navigationViewController!
      ...
      final updatedWaypointMarker =
          _newWaypointMarker!.copyWith(options: markerOptions);
      final updatedMarkers = await _navigationViewController!
  ...
  Future<void> _convertNewWaypointMarkerToDestinationMarker(
    ...
    final title = 'Waypoint $index';
    final waypointMarkerImage = await registerWaypointMarkerImage(
      index,
      MediaQuery.of(context).devicePixelRatio,
    );
    final destinationMarkers = await _navigationViewController!.updateMarkers(
      <Marker>[
  ...
  Future<void> _arrivedToWaypoint(NavigationWaypoint waypoint) async {
    ...
    if (_destinationWaypointMarkers.isNotEmpty) {
      final markerToRemove = _destinationWaypointMarkers.first;
  ...
  Future<bool> _updateNavigationDestinations() async {
    ...
    final destinations = _routeTokensEnabled
    ...
    try {
      ...
      switch (navRouteStatus) {
        ...
        case NavigationRouteStatus.quotaExceeded:
          showMessage(
            'Insufficient API quota to use the navigation.',
          );
        ...
        case NavigationRouteStatus.apiKeyNotAuthorized:
          showMessage(
            'A valid API key is required to use the navigation.',
          );
        ...
        case NavigationRouteStatus.noWaypointsError:
          showMessage(
            'The route could not be calculated because '
            'no waypoints were provided.',
          );
        ...
        case NavigationRouteStatus.locationUnknown:
          showMessage(
            'The route could not be calculated, because the user '
            'location is unknown.',
          );
      }
  ...
  Future<Destinations?> _buildDestinationsWithRoutesApi() async {
    assert(_routeTokensEnabled, 'can be null');
    ...
    if (routeTokens.isEmpty) {
      ...
      showMessage(
        'More than one route token received from Routes API. '
        'Using the first one.',
      );
    }
  ...
  Future<void> _resumeSimulation() async {
    assert(_simulationState == SimulationState.paused, 'can be null');
  ...
  Future<void> _displayTraveledRoute() async {
    final route = await GoogleMapsNavigator.getTraveledRoute();
    showMessage('Traveled route segment points: ${route.length}');
  }

  Future<void> _displayCurrentRouteSegment() async {
    final segment = await GoogleMapsNavigator.getCurrentRouteSegment();
    showMessage('Current route segment destination: '
        '${segment?.destinationWaypoint?.title ?? 'unknown'}');
  }
    ...
    if (!_termsAndConditionsAccepted || !_locationPermissionsAccepted) {
      return Padding(
        ...
        child: Wrap(
          ...
          spacing: 10,
          children: <Widget>[
            const Text('Terms and conditions and location permissions '
                'must be accepted'
  ...
  Widget _createRemainingTimeAndDistanceLabels() {
    final rTime = formatRemainingDuration(Duration(seconds: _remainingTime));
    final rDistance = formatRemainingDistance(_remainingDistance);
    return SafeArea(
            ...
            children: <Widget>[
              ...
              Card(
                child: ListTile(
                  ...
                  trailing: Text(
                    _onTrafficUpdatedEventCallCount.toString(),
                  ),
                ),
              ),
              Card(
  ...
  Widget buildOverlayContent(BuildContext context) {
    Color? getExpansionTileTextColor({required bool disabled}) {
            ...
            children: <Widget>[
              Wrap(
                ...
                children: <Widget>[
                  ...
                  ElevatedButton(
                    onPressed: _termsAndConditionsAccepted ? _resetTOS : null,
            ...
            children: <Widget>[
              Wrap(
                ...
                children: <Widget>[
                  ElevatedButton(
                    onPressed:
                        !_navigatorInitialized ? _initializeNavigator : null,
                    child: const Text('Start navigation'),
                  ),
                  ElevatedButton(
                    onPressed:
                        _navigatorInitialized ? _stopGuidedNavigation : null,
                  ...
                  ExampleSwitch(
                    ...
                    onChanged: _guidanceRunning ? null : _setRouteTokensEnabled,
                  ),
                ],
              ),
              const SizedBox(height: 10),
        ...
        IgnorePointer(
          ...
          child: Card(
            child: ExpansionTile(
              ...
              collapsedTextColor:
                  getExpansionTileTextColor(disabled: !_navigatorInitialized),
              collapsedIconColor:
                  getExpansionTileTextColor(disabled: !_navigatorInitialized),
              children: <Widget>[
                Wrap(
  ...
  Widget _buildTravelModeChoice(NavigationTravelMode mode, IconData icon) {
    final isSelected = mode == _travelMode;
    final enabled =
